...
abstract class IPostService {
  ...
  Future<List<Comments>?> fetchRelatedCommentsWithPostId(int postId);
}
...
class PostService implements IPostService {
  ...
  @override
  Future<List<Comments>?> fetchRelatedCommentsWithPostId(int postId) async {
    try {
      ...
      if (response.statusCode == HttpStatus.ok) {
        ...
        if (_datas is List) {
          return _datas.map((e) => Comments.fromJson(e)).toList();
        }
      }
    } on DioError catch (exception) {
      _ShowDebug.showDioError(exception, this);
